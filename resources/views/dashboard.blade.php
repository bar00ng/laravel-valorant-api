...
<div class="container mx-auto px-4 pt-10">
  ...
  <div class="maps py-20">
    <h2 class="uppercase tracking-wider text-red-500 text-xl font-semibold text-center">maps</h2>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-0 mt-5">
      @foreach ($maps as $map)
        @if ($loop->index < 6)
            <div class="relative group hover:cursor-pointer hover:scale-110 transation duration-100 z-10 hover:z-20">
    ...
    <a href="{{ route('maps') }}" class="mt-5">
      <h2 class="mt-6 text-red-500 text-center text-sm hover:underline hover:underline-offset-4">see more maps</h2>
    </a>
  </div>
  ...
  <div class="bundles py-10">
    <h2 class="uppercase tracking-wider text-red-500 text-xl font-semibold text-center">Bundles</h2>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-0 mt-5">
      @foreach ($bundles as $bundle)
        @if ($loop->index < 7)
          ...
          @endif
            <div class="relative group hover:cursor-pointer hover:scale-110 transation duration-100 bg-white z-10 hover:z-20">
              <img src="{{$bundle['displayIcon']}}" alt="{{$bundle['displayName']}}" class="opacity-50 group-hover:opacity-75">
              <div class="absolute top-0 left-0 flex items-center justify-center w-full h-full uppercase font-bold text-lg tracking-wider group bg-black bg-opacity-50">
                <h1 class="group-hover:text-red-500">{{$bundle['displayName']}}</h1>
              </div>
            </div>
          @endif
      @endforeach
    </div>
    <a href="{{ route('bundles') }}" class="mt-5">
      <h2 class="mt-6 text-red-500 text-center text-sm hover:underline hover:underline-offset-4">see more bundles</h2>
    </a>
  </div>
